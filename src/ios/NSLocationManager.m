...
@implementation NSLocationManager

+ (NSLocationManager *) sharedGpsManager {
    static NSLocationManager *shared = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared = [[NSLocationManager alloc] init];
    });
    return shared;
}
...
+ (void) getMoLocationWithSuccess:(MoLocationSuccess)success Failure:(MoLocationFailed)failure {
    [[NSLocationManager sharedGpsManager] getMoLocationWithSuccess:success Failure:failure];
}
...
+ (void) stop {
    [[NSLocationManager sharedGpsManager] stop];
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
    for (CLLocation *loc in locations) {
        ...
        //垂直高度
        double ver = loc.verticalAccuracy;
        successCallBack ? successCallBack(lat, lnt ,alt) : nil;
    }
}
